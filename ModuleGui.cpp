...
void ModuleGui::Draw() {
	...
	if (draw_hierarchy) {
		...
		for (std::vector<GameObject*>::iterator it = App->scene_intro->root_game_objects.begin(); it != App->scene_intro->root_game_objects.end(); it++) {
			(*it)->HierarchyTree(App->scene_intro->selected_game_obj_uid);
		}
